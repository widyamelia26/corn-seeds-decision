...
def main():
    ...
    if method == "AHP":
        ...
        weighting_method = st.radio("Pilih metode pembobotan kriteria:", ["Subjektif (Preferensi Pribadi)", "Bantuan AHP"], horizontal=True)

        # ---------- PEMBOBOTAN SUBJEKTIF ----------
        if weighting_method == "Subjektif (Preferensi Pribadi)":
        ...
        # ---------- PEMBOBOTAN AHP ----------
        elif weighting_method == "Bantuan AHP":
            ...
            col1, col2 = st.columns(2)
            for i in range(num_criteria):
                if i % 2 == 0:  # Jika indeks genap, tempatkan di kolom pertama
                    with col1:
                        is_benefit = st.selectbox(f"Tipe untuk {criteria[i]['name']}", ["Benefit", "Cost"], key=f"benefit_{i}")
                        is_benefit_list.append(is_benefit == "Benefit")
                else:  # Jika indeks ganjil, tempatkan di kolom kedua
                    with col2:
                        is_benefit = st.selectbox(f"Tipe untuk {criteria[i]['name']}", ["Benefit", "Cost"], key=f"benefit_{i}")
                        is_benefit_list.append(is_benefit == "Benefit")
        ...
        if st.button("\U0001F50E Tentukan Varietas Jagung dengan TOPSIS"):
            try:
                ...
                scores, ranking, Y, A_plus = topsis(matrix, weights, is_benefit_list)
                ...
                # Ambil peringkat 3 teratas
                top_n = min(3, len(result_df))  # antisipasi jika alternatif < 3
                top3_df = result_df.iloc[:top_n]

                alt_names = top3_df["Alternatif"].tolist()
                scores = top3_df["Skor TOPSIS"].tolist()

                # Ambil selisih absolut antara Y dan A_plus
                selisih_df = pd.DataFrame(Y - A_plus, columns=[c["name"] for c in criteria], index=alternatives).abs()

                # Ambil 3 kriteria terbaik dari masing-masing alternatif (gap terkecil ke ideal positif)
                def get_top_kriteria(selisih_row, max_k=3):
                    available_k = min(len(selisih_row), max_k)  # antisipasi jika kriteria < 3
                    zero_diff = selisih_row[selisih_row == 0.0]
                    
                    if len(zero_diff) >= available_k:
                        return zero_diff.index.tolist()[:available_k]
                    else:
                        return selisih_row.sort_values().index[:available_k].tolist()

                top_kriteria_dict = {}
                for i, alt in enumerate(alt_names):
                    top_kriteria_dict[alt] = get_top_kriteria(selisih_df.loc[alt])

                # Tampilkan kesimpulan
                st.markdown("### \U0001F4DD Kesimpulan")

                st.markdown(f"""
                Berdasarkan hasil perhitungan dengan metode *TOPSIS*, varietas **{alt_names[0]}** direkomendasikan sebagai pilihan utama karena memiliki skor tertinggi dan paling mendekati solusi ideal. Varietas ini menunjukkan performa unggul pada kriteria: {', '.join(f"**{k}**" for k in top_kriteria_dict[alt_names[0]])}, sehingga menjadi pilihan yang tepat untuk mencapai hasil panen yang optimal.
                """)

                if len(alt_names) > 1:
                    alt_markdown_lines = []
                    for i in range(1, len(alt_names)):
                        nama = alt_names[i]
                        kriteria = ', '.join(f"**{k}**" for k in top_kriteria_dict[nama])
                        alt_markdown_lines.append(f"- **{nama}**, dengan keunggulan pada kriteria: {kriteria}.")

                    full_alt_markdown = "\n".join(line.strip() for line in alt_markdown_lines)

                    st.markdown(
                    f"""Selain varietas utama tersebut, varietas berikut juga layak dipertimbangkan:\n\n{full_alt_markdown}
                    """)
        ...
        weighting_method = st.radio("", ["Subjektif (Preferensi Pribadi)", "Bantuan AHP"], horizontal=True)
        ...
        if st.button("\U0001F50E Tentukan Varietas Jagung Terbaik dengan Profile Matching"):
            ...
            result_df, gap_weights = profile_matching(criteria, alternatives, scores)
            ...
            top_n = min(3, len(result_df))  # Antisipasi jika alternatif < 3
            top_df = result_df.iloc[:top_n]

            # Ambil nama dan skor varietas teratas
            alt_names = top_df["Varietas"].tolist()
            scores = top_df["Total Score"].tolist()

            # Fungsi untuk ambil kriteria terbaik (adaptif ke jumlah kriteria)
            def get_top_kriteria(gap_row, max_k=3):
                gap_row = gap_row.astype(float)
                k = min(max_k, len(gap_row))
                return gap_row.sort_values(ascending=False).index[:k].tolist()

            # Ambil kriteria unggulan untuk masing-masing varietas
            top_kriteria_dict = {}
            for alt in alt_names:
                top_kriteria_dict[alt] = get_top_kriteria(gap_weights.loc[alt])

            # --- Tampilkan Kesimpulan dan Rekomendasi ---
            st.success(f"Varietas terbaik adalah **{alt_names[0]}** dengan skor {scores[0]:.4f}.")

            st.markdown("### \U0001F4DD Kesimpulan")

            st.markdown(f"""
            Berdasarkan hasil perhitungan dengan metode *Profile Matching*, varietas **{alt_names[0]}** direkomendasikan sebagai pilihan utama karena memiliki skor total tertinggi dan paling mendekati profil ideal. Varietas ini menunjukkan performa yang unggul pada kriteria: {', '.join(f"**{k}**" for k in top_kriteria_dict[alt_names[0]])}, sehingga menjadi pilihan yang tepat untuk mencapai hasil yang optimal sesuai dengan kondisi lahan dan tujuan petani.
            """)

            if top_n > 1:
                # Baris pembuka
                header = "Selain varietas utama tersebut, varietas berikut juga layak dipertimbangkan sebagai alternatif sesuai kondisi lahan:\n"

                # Gabungkan semua baris alternatif ke list
                alt_lines = []
                for i in range(1, top_n):
                    nama = alt_names[i]
                    kriteria = ', '.join(f"**{k}**" for k in top_kriteria_dict[nama])
                    alt_lines.append(f"- **{nama}**, dengan keunggulan pada kriteria: {kriteria}.")

                # Gabungkan jadi satu string dengan newline
                full_markdown = header + "\n".join(alt_lines)

                # Tampilkan 1 markdown saja
